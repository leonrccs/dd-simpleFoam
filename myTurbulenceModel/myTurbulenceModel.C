...
void myTurbulenceModel<BasicTurbulenceModel>::correctNut()
{
...
template<class BasicTurbulenceModel>
tmp<fvScalarMatrix> myTurbulenceModel<BasicTurbulenceModel>::kSource() const
{
...
template<class BasicTurbulenceModel>
tmp<fvScalarMatrix> myTurbulenceModel<BasicTurbulenceModel>::epsilonSource() const
{
    return tmp<fvScalarMatrix>
    (
        new fvScalarMatrix
...
:
    ...
    // wall_blending_(this->coeffDict_.get<Switch>("wall_blending")),
    k_
    (
        IOobject
        (
    ...
    (
        ...
        this->mesh_,
        dimensionedSymmTensor("a_ml", dimensionSet(0, 2, -2, 0, 0, 0, 0), symmTensor(.0, .0, .0, .0, .0, .0))
    ...
    (
        ...
        this->mesh_,
        dimensionedScalar("gamma_wall", dimless, 1.0)
    )
{
    bound(k_, this->kMin_);
...
bool myTurbulenceModel<BasicTurbulenceModel>::read()
{
    ...
    {
        wall_blending_.readIfPresent("wall_blending", this->coeffDict());
        ...
        gamma_mix_it_.readIfPresent(this->coeffDict());
...
void myTurbulenceModel<BasicTurbulenceModel>::correct()
{
    ...
    const volScalarField::Internal G_ml(
        this->GName(),
        tgradU().v() && (this->gamma_wall.v()*this->a_ml.v())
    );
    ...
    (
     ...
     ==
        ...
        //(1 - this->gamma_wall)*alpha()*rho()*G
      //- this->gamma_wall*alpha()*rho()*G_ml
...
template<class BasicTurbulenceModel>
tmp<fvVectorMatrix>myTurbulenceModel<BasicTurbulenceModel>::divDevRhoReff
(
    volVectorField& U
) const
{
    Info << "Calculating devReff from myTurbulenceModel" << endl;

    label timeIndex = this->mesh_.time().timeIndex();
    label startTime = this->runTime_.startTimeIndex();
    if (timeIndex - 1 == startTime){
        // compute anisotropy tensor from rans
        Info << "Computing a_rans ... " << endl;
        // this->a_rans = -1*this->alpha_*this->rho_*this->nut_*(ident_ & twoSymm(fvc::grad(U)));
        this->a_rans = -1*this->alpha_*this->rho_*this->nut_*(twoSymm(fvc::grad(U)));
        Info << "... done!" << endl;

        // compute anisotropy tensor from ml_prediction
        Info << "Computing a_ml ..." << endl;
    ...
    if (wall_blending_)
    {
        Info << "Calculate wall blending coefficients" << endl;
            // Mixing field
        ...
        dimensionedScalar alpha2 = dimensionedScalar("alpha2", dimless, 0.005);
        dimensionedScalar small0 = dimensionedScalar("small", dimless, 1e-10);

        // calculate wall blending coefficient
        this->gamma_wall = min(alpha1, pow(tanh((d/(alpha2*lambdb0 + small0))), 2));
    }

    // blend a_rans and a_ml
    Info << "Blending a_rans and a_ml ..." << endl;
    ...
    //   - fvm::laplacian(this->alpha_*this->rho_*this->nu(), U)
    //   - fvc::div((this->alpha_*this->rho_*this->nu())*dev2(T(fvc::grad(U))))
    //   - fvm::laplacian(this->alpha_*this->rho_*this->nut_*(1-this->gamma_wall), U)
    //   - fvc::div((this->alpha_*this->rho_*this->nut_)
    //   *dev2(T(fvc::grad(U)))*(1-this->gamma_wall))
    //   + fvc::div(this->gamma_wall*this->a_ml)
    // Inlcuding data driven anisotropy tensor and and holding all but U and p constant
      - fvm::laplacian(this->alpha_*this->rho_*this->nu(), U)
      - fvc::div((this->alpha_*this->rho_*this->nu())*dev2(T(fvc::grad(U))))
      + fvc::div(this->a_mix)
    );
